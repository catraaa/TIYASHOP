...
export default function AdminView() {
  ...
  return (
    <section>
      <div className="bg-white relative dark:bg-white mx-auto px-4 sm:px-6 lg:px-8">
        <div>
                ...
                <ul className="flex flex-col gap-4">
                  {allOrdersForAllUsers.map((item) => (
                    <li key={item._id} className="bg-gray-200 shadow p-5 flex flex-col space-y-3 py-6 text-left">
                      <div className="flex">
                        <h1 className="font-bold text-lg mb-3 flex-1 text-black">#order: {item._id}</h1>
                        <div className="flex flex-col gap-2">
                          <div className="flex items-center">
                            <p className="mr-3 text-sm font-medium text-gray-900">Nama Pembeli :</p>
                            <p className="text-sm  font-semibold text-gray-900">{item?.user?.name}</p>
                          </div>
                          <div className="flex items-center">
                            <p className="mr-3 text-sm font-medium text-gray-900">Email Pembeli :</p>
                            <p className="text-sm  font-semibold text-gray-900">{item?.user?.email}</p>
                          </div>
                          <div className="flex items-center">
                            <p className="mr-3 text-sm font-medium text-gray-900">Jumlah Total yang Dibayar :</p>
                            <p className="text-sm  font-semibold text-gray-900">Rp{item?.totalPrice}</p>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="flex gap-5">
                        <button className="disabled:opacity-50 mt-5 mr-5  inline-block bg-blue-900 rounded-full text-white px-5 py-3 text-xs font-medium uppercase tracking-wide">
                          {item.isProcessing ? "Pesanan Sedang Diproses" : "Pesanan dikirimkan"}
                        </button>
                        <button
                          onClick={() => handleUpdateOrderStatus(item)}
                          disabled={!item.isProcessing}
                          className="disabled:opacity-50 mt-5 mr-5  inline-block bg-blue-900 rounded-full text-white px-5 py-3 text-xs font-medium uppercase tracking-wide"
                        >
                          {componentLevelLoader && componentLevelLoader.loading && componentLevelLoader.id === item._id ? (
                            <ComponentLevelLoader text={"Memperbarui Status Pesanan"} color={"#ffffff"} loading={componentLevelLoader && componentLevelLoader.loading} />
                          ) : (
