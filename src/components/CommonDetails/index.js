...
export default function CommonDetails({ item }) {
  ...
  return (
    <section className="relative dark:bg-white text-white dark:text-black mx-auto max-w-screen-xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="lg:col-span-2 lg:row-span-2 lg:row-end-2">
            ...
            <div className="mt-10 flex flex-col items-center justify-between space-y-4 botder-t border-b py-4 sm:flex-row sm:space-y-0">
              <div className="flex items-end">
                <h1 className={`text-[20px] font-bold mr-2 Rp{item.onSale === "yes" ? "line-through" : ""}`}>Rp {item && item.price.toLocaleString("id-ID")}</h1>
                {item.onSale === "yes" ? <h1 className="text-[20px] font-bold text-red-700">Rp {(item.price - item.price * (item.priceDrop / 100)).toLocaleString("id-ID", { maximumFractionDigits: 0 })}
</h1> : null}
              </div>

              <button type="button" onClick={() => handleAddToCart(item)} className="mt-1.5 inline-block bg-blue-900 px-1 py-3 text-xs font-medium tracking-wide uppercase text-white">
                {componentLevelLoader && componentLevelLoader.loading ? <ComponentLevelLoader text={"Ditambahkan ke keranjang"} color={"#ffffff"} loading={componentLevelLoader && componentLevelLoader.loading} /> : "Masukkan Keranjang"}
              </button>
            </div>
            <ul className="mt-8 space-y-2">
              <li className="flex items-center text-left text-sm font-medium text-gray-600">{item && item.deliveryInfo}</li>
              <li className="flex items-center text-left text-sm font-medium text-gray-600">{"Batalkan kapan saja"}</li>
            </ul>
